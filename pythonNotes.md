...
>
> - Step 1: open VS Code with the repo folder.
>   - System: VS Code app.
>   - Path: `/Users/coder/Public/EngineGit/pythonCatchup/`.
>   - Verify: Explorer sidebar shows files like `pythonNotes.md`, `pyproject.toml`, `scripts/`, `src/`, and `tests/`.
>
> - Step 2: open the Extensions panel.
>   - System: VS Code UI.
>   - Path: left Activity Bar square-block Extensions icon, or `Cmd + Shift + X`.
>   - Verify: the Extensions search box is visible.
>
> - Step 3: install Python extension `ms-python.python`.
>   - System: VS Code extension marketplace.
>   - Path: Extensions search box.
>   - Verify: extension by Microsoft is installed and the Command Palette contains `Python: Select Interpreter`.
>
> - Step 4: install Ruff extension `charliermarsh.ruff`.
>   - System: VS Code extension marketplace.
>   - Path: Extensions search box.
>   - Verify: extension by Astral Software is installed and Python files can use Ruff as formatter.
>
> - Step 5: install Jupyter extension `ms-toolsai.jupyter` only if using notebooks.
>   - System: VS Code extension marketplace.
>   - Path: Extensions search box.
>   - Verify: `.ipynb` files open with notebook support.
>
> - Step 6: reload VS Code if extension behavior does not appear.
>   - System: VS Code extension host.
>   - Path: Command Palette command `Developer: Reload Window`.
>   - Verify: Python commands and Ruff formatting options appear after reload.

> [workflow] Install recommended VS Code extensions on macOS from `zsh`
>
> - Step 1: open the VS Code integrated terminal.
>   - System: VS Code terminal.
>   - Path: menu `Terminal` -> `New Terminal`, using `zsh`.
>   - Verify: `pwd` prints `/Users/coder/Public/EngineGit/pythonCatchup`.
>
> - Step 2: check whether the `code` command is available with `code --version`.
>   - System: VS Code command-line interface.
...
>
> - Step 3: install Python extension with `code --install-extension ms-python.python`.
>   - System: VS Code extension installer.
>   - Path: VS Code user extensions folder.
>   - Verify: command reports installed or already installed.
>
> - Step 4: install Ruff extension with `code --install-extension charliermarsh.ruff`.
>   - System: VS Code extension installer.
>   - Path: VS Code user extensions folder.
>   - Verify: command reports installed or already installed.
>
> - Step 5: install Jupyter only if needed with `code --install-extension ms-toolsai.jupyter`.
>   - System: VS Code extension installer.
>   - Path: VS Code user extensions folder.
>   - Verify: command reports installed or already installed when notebook work is needed.
>
> - Step 6: list relevant installed extensions with `code --list-extensions --show-versions | rg "ms-python.python|charliermarsh.ruff|ms-toolsai.jupyter"`.
>   - System: VS Code extension registry.
>   - Path: VS Code user extensions folder.
>   - Verify: expected extension IDs and versions appear.
>
> - Step 7: confirm Python and Ruff are visible inside VS Code.
>   - System: VS Code extension host.
>   - Path: Command Palette and current `.py` file.
>   - Verify: `Python: Select Interpreter` appears, and `Format Document With...` includes Ruff for Python files.
...
> [execution] macOS VS Code extension verification
>
> - Command: `pwd`
> - Result: `/Users/coder/Public/EngineGit/pythonCatchup`.
> - Command: `code --version`
> - Result: initially failed because `code` was not on PATH.
> - Check: VS Code app exists at `/Applications/Visual Studio Code.app`.
> - Command: `/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code --version`
> - Result: VS Code CLI available; version `1.135.0`, commit `08d4889f9ec4a1685d257b9b95de036c8e1ce1e5`, arch `arm64`.
> - Command: `/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code --install-extension ms-python.python`
> - Result: `ms-python.python@2026.4.0` already installed.
> - Command: `/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code --install-extension charliermarsh.ruff`
> - Result: `charliermarsh.ruff@2026.74.0` installed.
> - Command: `ln -s /Applications/Visual Studio Code.app/Contents/Resources/app/bin/code /opt/homebrew/bin/code`
> - Result: normal `code` command is now available on PATH.
> - Command: `which code`
> - Result: `/opt/homebrew/bin/code`.
> - Command: `code --list-extensions --show-versions | rg 'ms-python.python|charliermarsh.ruff|ms-toolsai.jupyter|ms-python.vscode-pylance'`
> - Result: found `charliermarsh.ruff@2026.74.0`, `ms-python.python@2026.4.0`, and `ms-python.vscode-pylance@2026.3.1`.
> - Jupyter: not installed because it is optional until notebook work is needed.
